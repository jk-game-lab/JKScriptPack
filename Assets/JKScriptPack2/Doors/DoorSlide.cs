...
namespace JKScriptPack2
{
    ...
    public class DoorSlide : MonoBehaviour
    {
        ...
        public GameObject oppositeDoor = null;
        ...
        void Start()
        {
            ...
            if (mainDoor)
            {
                doorOrigin = mainDoor.transform.position;
                doorDestination = mainDoor.transform.TransformPoint(slide);
            }
            if (oppositeDoor)
            {
                secondDoorOrigin = oppositeDoor.transform.position;
                secondDoorDestination = oppositeDoor.transform.TransformPoint(-slide);
            }

            // Set up audio
            if (mainDoor)
            {
                audiosource = mainDoor.AddComponent<AudioSource>();
            }
            else
            {
        ...
        void Update()
        {
            ...
            if (mainDoor)
            {
                mainDoor.transform.position = Vector3.Lerp(doorOrigin, doorDestination, travel);
            }
            if (oppositeDoor)
            {
                oppositeDoor.transform.position = Vector3.Lerp(secondDoorOrigin, secondDoorDestination, travel);
            }
